<script>
    ...
    function initEditLedger(title, id, callback) {
        ...
        win.showPost("Edit Ledger - " + title, "{{ route('admin.ledger.edit') }}", {
            "id": id
        }, addEXPayHandle);
    ...
    function getUserBalance($id) {
        return new Promise((resolve, reject) => {
            axios.post('{{ route('admin.ledger.balance') }}', {
                    id: $id
                })
                .then((res) => {
                    resolve(parseFloat(res.data.balance));
                })
                .catch((err) => {
                    reject(err);
                })
    ...
    function getDRCR(amount) {
        return "Rs. " + (amount == 0 ? '0' : (amount < 0 ? ((-1 * amount).toString() + " Cr.") : (amount.toString() +
            " Dr.")));
    }
