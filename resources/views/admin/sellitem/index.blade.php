                ...
                <div class="row d-none" id="bill">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <input type="text" list="farmerdatalist" name="user_id" id="u_id"
                                placeholder="User number" class="form-control checkfarmer next  " data-next="item_id"
                                min="1">
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="item_id" list="itemdatalist" name="number"
                                placeholder="Item number" class="form-control {{ !$large ? 'checkitem' : '' }} next "
                                data-rate="rate" data-next="rate" min="1">
                        </div>
                    </div>
                    @if(env('use_sell_item_batch'))

                        <div class="col-md-3">
                            <label for="batch_id"><input type="checkbox" name="use_batch" id="use_batch"> Batch</label>
                            <select name="batch_id" id="batch_id" class="form-control ms"></select>
                        </div>

                    @endif
                    <div class="col-md-2">
                        <label for="rate">Rate</label>
                        <input type="number" name="rate" onkeyup="calTotal(); paidTotal();" id="rate"
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="paid" onkeyup="paidTotal();" id="paid" step="0.001"
                            placeholder="Paid" value="0" class="form-control xpay_handle" min="0.001">
                    </div>

                    <div class="col-md-3">
                        <label for="due">Due</label>
                        <input type="number" name="due" id="due" step="0.001" placeholder="due"
                            value="0" class="form-control" min="0" readonly>
                    </div>
                ...
                <div class="col-md-12">
                    <div class="mt-5">
                        ...
                        <table id="newstable1" class="table table-bordered">
                            <thead>
                                <tr>
                                    <th><input type="checkbox" id="allchecked" onchange="selectAll(this.checked);">
                                        Farmer No.</th>
                                    <th>Item Name</th>
                                    <th>Rate</th>
                                    <th>Quantity</th>
...
@section('js')
    ...
    <script>
        ...
        function saveData() {
            if ($('#nepali-datepicker').val() == '' || $('#u_id').val() == '' || $('#item_id').val() == '' || $('#total')
                .val() == 0) {
                ...
                @if(env('use_sell_item_batch'))
                    ...
                    if($('#use_batch')[0].checked){
                        const batch_id=$('#batch_id').val();

                        const batch=batches.find(o=>o.batch_id==batch_id);
                        ...
                        const qty=$('#qty').val();
                        if(qty>batch.amount){
                            alert('Not enough stock in batch');
                            return;
                        }
                    }
                @endif
        ...
        function itemSelected(data) {
            ...
            @if ($large) $('#item_id').closeSearch(); @endif
        ...
        function loadData() {
            ...
            axios.post('{{ route('admin.sell.item.list') }}', {
                    'center_id': center_id,
            ...
            if (hasBatches.includes(id)) {
                ...
                axios.get(batchURL.replace('xxx_id', id))
                    .then((res) => {
                        ...
                        $('#batch_id').html(
                            batches.map(o =>
                                `<option value="${o.batch_id}">${o.batch_no} (${o.amount.toString() } )</option>`
                                ).join('')
                        );
                        $('#use_batch')[0].checked = batches.length>0;
        ...
        <script>
            function renderBarcode() {
                ...
                this.data.forEach(item => {
                    html += '<tr class="search-item" id="item-' + dotSanitize(item.number) + '" data-rate="' + item
                        .sell_price + '" data-number="' + item.number + '" data-name="' + item.title +
                        '" onclick="itemSelected(this.dataset);">' +
