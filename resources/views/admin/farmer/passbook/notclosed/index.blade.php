...
@section('title', 'passbook - not closed')
@section('css')
    <link rel="stylesheet" href="{{ asset('calender/nepali.datepicker.v3.2.min.css') }}" />
@endsection
@section('head-title')
    <a href="{{ route('admin.farmer.passbook.index') }}">Passbook Check</a> / Not Closed List
@endsection
@section('toobar')
...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <span class="btn btn-primary" onclick="loadData()"> Load List</span>
...
@section('js')
    ...
    <script>
        ...
        function saveData(e, ele) {
            ...
            if (prompt("Enter yes to continue") == 'yes') {
                ...
                axios.post(ele.action, new FormData(ele))
                    .then((res) => {
                        hideProgress();
                        $('#allData').html("Data Saved Successfully");
        ...
        function loadData() {
            ...
            axios.post("{{ route('admin.farmer.passbook.notClosed') }}", d)
                .then(function(response) {
                    hideProgress();
                    $('#allData').html(response.data);
                    var edit = document.getElementById("date");
                    if (edit != undefined) {
        ...
        function printDiv(id) {
            ...
            newWin.document.write(
                '<html><head><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/css/bootstrap.min.css" integrity="sha384-TX8t27EcRE3e/ihU7zmQxVncDAy5uIKz4rEkgIXeMed4M0jlfIDPvg6uqKI2xXr2" crossorigin="anonymous"><link rel="stylesheet" href="{{ asset('backend/css/print.css') }}"></head><body onload="window.print()">' +
