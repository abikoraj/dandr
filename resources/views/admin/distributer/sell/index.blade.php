        ...
        <div class="col-md-9 bg-light pt-2">
            <form action="" id="sellitemData">
                ...
                <div class="row">
                                    ...
                                    <select name="center_id" id="center_id" class="ms form-control">
                                        @foreach ($centers as $center)
                                            <option value="{{ $center->id }}"
                                                {{ isMainCenter($center->id) ? 'selected' : '' }}>{{ $center->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="product_id"><span id="productsearch" data-toggle="modal"
                                    data-target="#itemmodal">{{ env('use_barcode', false) ? 'Barcode' : 'Product' }} ( search
                                    )</span></label>
                            <input type="{{ env('use_barcode', false) ? 'text' : 'number' }}" id="product_id" name="product_id"
                                class="form-control next" data-next="rate">
                        </div>
                    </div>

                    <div class="col-md-2">
                        <label for="rate">Rate</label>
                        <input type="number" name="rate" oninput="calTotal(); paidTotal();" id="rate" step="0.001" value="0"
                            placeholder="Item rate" class="form-control  next focus-select" data-next="qty" min="0.001">
                    </div>

                    <div class="col-md-2">
                        <label for="qty">Quantity</label>
                        <input type="number" onfocus="$(this).select();" name="qty" id="qty"
                            oninput="calTotal(); paidTotal();" step="0.001" value="1" placeholder="Item quantity"
                            class="form-control  next" data-next="paid" min="0.001">
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="paid" oninput="paidTotal();" id="paid" step="0.001" placeholder="Paid"
                            value="0" class="form-control focus-select xpay_handle" min="0.001">
                    </div>

                    <div class="col-md-3">
                        <label for="due">Due</label>
                        <input type="number" name="due" id="due" step="0.001" placeholder="due" value="0"
    ...
    <script>
        ...
        function saveData() {
            if (!sellock) {
                ...
                if ($('#nepali-datepicker').val() == '' || $('#id').val() == '' || $('#product_id').val() == '' || $(
                        '#total').val() == 0) {
                    ...
                    axios({
                        ...
                        .then(function(response) {
                            if (response.data == 'noStock') {
                                showNotification('bg-danger', 'item Stock is not available !');
                                sellock = false;
                            } else {
        ...
        function loaddata() {
            ...
            // list
            axios.post('{{ route('admin.distributer.sell.list') }}', {
    ...
    @if ($large)
        ...
        <script>
            function renderBarcode() {
                html = "<table>";
                this.data.forEach($i => {
                    html += '<tr class="search-item" id="item-' + ($i.dis_number ?? $i.number) + '" data-rate="' + (
                            $i.dis_price ?? $i.sell_price) + '" data-number="' + ($i.dis_number ?? $i.number) +
                        '" data-name="' + $i.title + '" onclick="itemSelected(this.dataset);">' +
                        '<td class="p-1"><span style="cursor: pointer;">' + ($i.dis_number ?? $i.number) +
                        '</span></td>' +
                        '</tr>';
                });
                html += "</table>"
