    ...
    @foreach ($centers as $center)
        <dl>
            ...
            <dd>
                ...
                Milk Quantity <input type="number" min="0"  id="amount_{{$center->id}}" step="0.01"  name="chalan_amount_{{$chalan->stock_out_item_id}}" value="{{$chalan->amount}}" required>
                @else
                <input type="hidden" name="center_ids[]" value="{{$center->id}}">
                Milk Quantity <input   id="amount_{{$center->id}}" type="number" min="0" step="0.01" name="center_amount_{{$center->id}}" >
